...
def get_default_optimizer_params(
    ...
    for module_name, module in model.named_modules():
        for module_param_name, value in module.named_parameters(recurse=False):
            ...
            ###### Use a small learning rate for pre-trained models
            if module_name.startswith("backbone.pretrained_"):
            # if module_name.split('.')[0] == "backbone" and module_name.split('.')[1].startswith("pretrained_"):
                hyperparams["lr"] = 0.000005  #hyperparams.get("lr", base_lr)*0.8 #ImageNet
                # print("Use a small learning rate for pre-trained models{}".format(module_name))
            ###### Use a 2x learning rate on the classification head
            if module_name.split('.')[0].startswith("head_") and heads_lr_factor not in (None, 1.0):
                hyperparams["lr"] = hyperparams.get("lr", base_lr) * heads_lr_factor
                # print("Use a 2x learning rate on the classification head{}".format(module_name))
            ###### Use a special learning rate for special module
            # if module_name.split('.')[0] == "inverseNet" and (heads_lr_factor is not None and heads_lr_factor != 1.0):
            #     hyperparams["lr"] = hyperparams.get("lr", base_lr) * heads_lr_factor

            name = module_name + '.' + module_param_name
            ###### The weight_decay for the scaling parameter is 0.
            if "scaling_factor" in name or "log_scaling_factor" in name:
