...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            #     act_outputs = []
            #
            #     def hook_fns_forward(module, input, caption):
            #         act_outputs.append(caption.cpu())
            #
            ...
            plt.tight_layout()
            # filepath = os.path.join(caption, "{}.jpg".format(cnt))
    ...
    def vis_rank_list(self, output, vis_label, num_vis=100, rank_sort="ascending", label_sort="ascending", max_rank=5,
        ...
        if len(query_indices) > 3000:
            query_indices = query_indices[2000:2000+int(num_vis)]
        else:
            query_indices = query_indices[int(len(query_indices)/2):int(len(query_indices)/2)+int(num_vis)]
    ...
    def vis_roc_curve(self, output):
        ...
        # filepath = os.path.join(caption, "pos_neg_dist.jpg")
        # plt.savefig(filepath)
