...
def build_dist(feat_1: torch.Tensor, feat_2: torch.Tensor, metric: str = "euclidean", **kwargs) -> np.ndarray:
    ...
    """
    assert metric in ["cosine", "euclidean", "jaccard", "hyperbolic"], "Expected metrics are cosine, euclidean, jaccard and hyperbolic" \
    ...
    if metric == "euclidean":
        ...
        return compute_hyperbolic_distance(feat_1, feat_2, curv=kwargs["curv"], input_in_tangent=True)
...
from ..modeling.backbones.manifolds.lorentz import pairwise_dist, exp_map0
@torch.no_grad()
def compute_hyperbolic_distance(
        feat_1: torch.Tensor,
        feat_2: torch.Tensor,
        curv: float = 1.0,
        input_in_tangent: bool = True,  # True: 先 exp_map0；False: 直接认为已经在双曲面
        eps: float = 1e-6,
) -> np.ndarray:
    # 1) 可选：从切空间映射到双曲面（space components）
    if input_in_tangent:
        x = exp_map0(feat_1, curv=curv, eps=eps)
        y = exp_map0(feat_2, curv=curv, eps=eps)
    else:
        x, y = feat_1, feat_2

    # 2) Lorentz 模型测地距离
    dist = pairwise_dist(x, y, curv=curv, eps=eps)
