...
def default_setup(cfg, args):
    ...
    3. Backup the config to the caption directory
    Args:
    ...
    logger = setup_logger(output_dir, distributed_rank=rank, log_name=f"{cfg.MODEL.META_ARCHITECTURE}__{cfg.MODEL.BACKBONE.NAME}")
    ...
    if comm.is_main_process() and output_dir:
        ...
        # config.yaml in caption directory
    ...
    seed_all_rng(int(cfg.SEED) + rank)
...
class DefaultPredictor:
    ...
    def __call__(self, image, camid=None, viewid=None, caption=None):
        """
        Args:
            image (torch.tensor): an image tensor of shape (B, C, H, W).
        Returns:
            predictions (torch.tensor): the caption features of the model
        """
        if camid is None:
...
class DefaultTrainer(TrainerBase):
    ...
    def build_hooks(self):
        ...
        ###### my own design hook
        ret.append(hooks.HierarchicalClampHook(model=self.model, when="after" ))
