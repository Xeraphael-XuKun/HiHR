...
class HierarchicalClampHook(HookBase):
    def __init__(self, model, when="after"):
        self._model = model
    ...
    def _clamp(self):
        model = self._get_real_model()
        if not hasattr(model, "backbone"):
            return
        backbone = getattr(model, "backbone")

        # 如果没有 clamp_curv，直接跳过
        if not hasattr(backbone, "clamp_curv"):
            return

        clamp_fn = getattr(backbone, "clamp_curv")
    ...
    def after_train(self):
        ...
        # 如果没有 clamp_curv，直接跳过
        if not hasattr(backbone, "log_scaling_factor_i1"):
            return

        curv = backbone.log_curv.exp()
        scaling_factor_i1 = torch.exp(backbone.log_scaling_factor_i1)
        scaling_factor_i2 = 1.0 + 1e-6 + torch.exp(backbone.log_scaling_factor_i2)

        logger = logging.getLogger(__name__)
        logger.info(f"curv is now {curv}")
        logger.info(f"scaling_factor_i1 is now {scaling_factor_i1}")
        logger.info(f"scaling_factor_i2 is now {scaling_factor_i2}")
...
class AutogradProfiler(HookBase):
    ...
    def __init__(self, enable_predicate, output_dir, *, use_cuda=True):
        ...
        Args:
            ...
            output_dir (str): the caption directory to dump tracing files.
